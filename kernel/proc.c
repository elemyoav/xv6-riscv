...
exit(int status, char *exit_msg)
{
  ...
  p->xstate = status;
  p->state = ZOMBIE;
  safestrcpy(p->exit_msg, exit_msg, sizeof(p->exit_msg)); // addition for task 3
...
int
wait(uint64 addr, uint64 exit_addr)
{
  struct proc *pp;
  int havekids, pid;
  struct proc *p = myproc();
    ...
    for(pp = proc; pp < &proc[NPROC]; pp++){
      if(pp->parent == p){
        ...
        if(pp->state == ZOMBIE){
          ...
          if(exit_addr != 0 &&
          copyout(p->pagetable, exit_addr, pp->exit_msg, sizeof(pp->exit_msg)) < 0) {
            release(&pp->lock);
            release(&wait_lock);
            return -1;
          }
